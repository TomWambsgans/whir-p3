    ...
    F::PrimeSubfield: TwoAdicField,
{
        ...
        [F::PrimeSubfield; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
    {
        ...
        let folded_matrix = info_span!("dft", height = evals_repeated.len() / width, width)
            .in_scope(|| {
                dft.dft_algebra_batch_by_evals(RowMajorMatrix::<F>::new(evals_repeated, width))
                    .to_row_major_matrix()
            });
        ...
        let extension_mmcs = ExtensionMmcs::<F::PrimeSubfield, F, _>::new(mmcs.clone());

        let (root, prover_data) =
            info_span!("commit_matrix").in_scope(|| extension_mmcs.commit_matrix(folded_matrix));
